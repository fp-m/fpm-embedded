...
static void remove_file(const char *filename, const options_t *options)
{
    if (!options->force) {
        ...
        uint16_t reply[32];
        fpm_snprintf(prompt, sizeof(prompt), "Remove file %s? y/n [n] ", filename);
        fpm_editline(reply, sizeof(reply), 1, prompt, 0);
        fpm_puts("\r\n");
        if (reply[0] != 'y' && reply[0] != 'Y') {
            fpm_printf("Not removed.\r\n");
            return;
        }
    }
...
static void remove_directory(const char *path, const options_t *options)
{
    if (!options->recursive) {
        if (!options->force) {
            fpm_printf("%s: Cannot remove directory without -r option\r\n", path);
        }
        return;
    }
    ...
    uint16_t reply[32];
    fpm_snprintf(prompt, sizeof(prompt), "Remove directory %s? y/n [n] ", path);
    fpm_editline(reply, sizeof(reply), 1, prompt, 0);
    fpm_puts("\r\n");
    if (reply[0] != 'y' && reply[0] != 'Y') {
        fpm_printf("Not removed.\r\n");
